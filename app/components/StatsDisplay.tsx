...
function StatCard({
  ...
  const statsContent = (
    <div className="bg-[#0a0f1a]/80 backdrop-blur-sm border border-[#1e3a5f] p-4 md:p-5 w-full min-h-[120px] h-full rounded-lg">
      <div className="space-y-2">
        <h2 className="my-0 font-mono font-medium text-xs tracking-tight uppercase text-gray-400 pr-6">
          {title}
        </h2>
        {baseValue !== undefined && (
          <div className="text-2xl md:text-3xl tracking-normal font-mono tabular-nums text-white">
            {formatNumber(value)}
          </div>
        )}
        {children}
  ...
  const infoContentView = (
    <div className="bg-[#0a0f1a]/95 backdrop-blur-sm border border-[#1e3a5f] p-4 md:p-5 w-full h-full overflow-y-auto flex flex-col gap-y-2 rounded-lg">
      {href ? (
        <a
          href={href}
          tabIndex={showInfo ? 0 : -1}
          className="my-0 font-mono font-medium text-xs tracking-tight uppercase text-[#4a90d9] hover:underline underline-offset-2 inline-flex gap-x-0.5 items-center w-fit shrink-0"
        >
          {title}
        ...
        <span className="my-0 font-mono font-medium text-xs tracking-tight uppercase text-white shrink-0">
          {title}
        </span>
      )}
  ...
  return (
    <div className={`relative group rounded-lg overflow-hidden ${className || ""}`}>
      ...
      />
      {infoContent && (
        <div className={`absolute top-3 right-3 transition-opacity duration-150 z-[20] isolate ${showInfo ? "opacity-100" : "opacity-100 md:opacity-0 md:group-hover:opacity-100 md:group-focus-within:opacity-100"}`}>
          <button
            aria-label={`Learn more about ${title}`}
            type="button"
            onClick={() => setShowInfo(!showInfo)}
            className="p-1 m-0 bg-transparent text-gray-500 border-none md:border md:border-solid border-[#1e3a5f] hover:text-white hover:bg-[#1e3a5f] transition-colors duration-150 flex items-center justify-center outline-none focus-visible:ring cursor-pointer rounded"
          >
            <InfoIcon />
          </button>
        </div>
...
function MetricRow({ label, baseValue, incrementRate, showRate = false }: { label: string; baseValue: number; incrementRate: number; showRate?: boolean }) {
  ...
  return (
    <li className="flex flex-wrap items-center justify-between gap-x-3">
      <h3 className="m-0 font-mono font-normal text-xs text-gray-500 uppercase">
        {label}
      </h3>
      <div className="flex items-center gap-3 md:gap-4 text-right">
        ...
        {showRate && (
          <div className="w-16 text-gray-500 text-right text-xs font-mono tabular-nums">
  ...
  return (
    <li className="flex items-center gap-3">
      ...
      <div className="flex-1 text-right">
        <span className="text-white text-sm font-mono tabular-nums">{formatNumber(value)}</span>
      </div>
      <div className="w-[80px] text-right text-gray-500 text-xs font-mono tabular-nums">
        <span>{formatNumber(rate)}</span>
        <span>/s</span>
      </div>
    </li>
...
export function TopCountries() {
  ...
  return (
    <div className="bg-[#0a0f1a]/80 backdrop-blur-sm border border-[#1e3a5f] rounded-lg p-4">
      <h2 className="my-0 font-mono font-medium text-xs tracking-tight uppercase text-gray-400 mb-3">
  ...
  return (
    <div className="bg-[#0a0f1a]/80 backdrop-blur-sm border border-[#1e3a5f] rounded-lg p-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-white font-mono font-semibold text-lg">19</span>
        <span className="text-gray-400 text-sm">Vercel Regions</span>
      </div>
    </div>
  );
}
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      ...
      <div className="flex flex-col gap-3">
        ...
        <StatCard 
          ...
          className="flex-1"
        >
          <p className="text-gray-500 text-sm font-mono mt-1">Cache hits served</p>
        </StatCard>
      </div>
    </div>
  );
